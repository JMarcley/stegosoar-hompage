...
function winAlert(player) {
	...
	if (player == "X") {
    $('.game-message').text("Congratulations, you beat the computer!");
	} else {
...
function checkDraw() {
    ...
    $('.game-message').text("Draw! Try playing again!");
		clearBoard();
	}
}
  ...
  if (win) {
    ...
    if (playValid) {
      ...
      Oplay();
          ...
          $('.game-message').text("That square has already been played. Please choose another square");
    }
  }
